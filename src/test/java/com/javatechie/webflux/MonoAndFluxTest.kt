...
class MonoAndFluxTest {
    ...
    @Test
    fun testMono1() {
        val monoString1: Mono<*> = Mono.just("javatechie")
            .log() //выводит все шаги в консоль
        monoString1.subscribe({ x: Any? -> println(x) }) { e: Throwable -> println(e.message) }
        //имеем такой вывод:
    ...
    @Test
    fun testMono2() {
        val monoString2: Mono<*> = Mono.just("javatechie")
            .then(Mono.error<Any>(RuntimeException("Exception occured")))
            .log() //выводит все шаги в консоль
        monoString2.subscribe({ x: Any? -> println(x) }) { e: Throwable -> println(e.message) }
    ...
    fun testFlux1() {
        //1
        val fluxString1 = Flux.just("Spring", "Spring Boot", "Hibernate", "microservice")
            .concatWithValues("AWS")
            .concatWithValues("cloud")
            .log()
        fluxString1.subscribe({ x: String? -> println(x) }) { e: Throwable -> println(e.message) }
        ...
        //1
        val fluxString2 = Flux.just("Spring", "Spring Boot", "Hibernate", "microservice")
            .concatWithValues("AWS")
            .concatWithValues("cloud")
            .log()
    ...
    fun testFlux2() {
        val fluxString2 = Flux.just("Spring", "Spring Boot", "Hibernate", "microservice")
            .concatWithValues("AWS")
            .concatWith(Flux.error(RuntimeException("Exception occured in Flux")))
            .concatWithValues("cloud")
            .log()
        fluxString2.subscribe()
